...
/**
 * ShadowDungeon class: The main game controller that manages rooms, player, input and rendering.
 */
public class ShadowDungeon extends AbstractGame {
     ...
     * @param messageProps UI/message strings configuration
     */
    ...
    protected void update(Input input) {
        ...
        if (pause) {
            // Pause overlay: draw current world + store UI
            ...
            if (input.wasPressed(Keys.L)) {
                ...
                store.buyForHealth(player);
            } else if (input.isDown(Keys.P)) {
                // Full reset during pause when 'P' is held
                pause = false;
                player.reset();
                prepRoom.reset();
                roomA.reset();
            ...
            // Shooting
            player.shotCoolDown();
            if (input.wasPressed(MouseButtons.LEFT) && player.getHasChosen()) {
                player.shot(currentRoom, input);
            }
            // Step simulation
            currentRoom.move();
            ...
            // Collisions & victory checks
            if (currentRoom == roomA) roomA.projectilesClashTest(player);
            if (currentRoom == roomB) roomB.projectilesClashTest(player);
            if (currentRoom == roomA && !roomA.getPassed()) roomA.winTest(player);
            if (currentRoom == roomB && !roomB.getPassed()) roomB.winTest(player);
            // Movement & world bounds
            player.setterBounds(player.getPlayerPos(input));
            if (currentRoom == roomA || currentRoom == roomB) {
                player.move(input, solid, tmpBasket, tmpTable,
                        currentRoom.getPriDoors(), currentRoom.getSecDoors(), currentRoom.getGateDelay());
            } else {
                player.move(input, currentRoom.getPriDoors(), currentRoom.getSecDoors(), currentRoom.getGateDelay());
            }
            // Room transitions / interactions
        ...
        // Toggle pause
        if (input.wasPressed(Keys.SPACE) && !pause) {
            pause = true;
        } else if (input.wasPressed(Keys.SPACE) && pause) {
            pause = false;
        }
    }
    /**
